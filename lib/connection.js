...
class Connection {
	...
	query( ...args )
	{
		// First parameter to Connection.query() can be a string, object, or function
		let sql;
		...
		// This check isn't meant to be bulletproof because it can't be.  Relies on convention of capitalizing SQL keywords.
		if( sql !== undefined && ( sql.startsWith( 'UPDATE' ) || sql.startsWith( 'DELETE' ) ) && !sql.includes( 'WHERE' ) )
		{
			return Promise.reject( new Error( 'detected UPDATE or DELETE query with no WHERE clause' ) );
		}

		return promiseCallback.apply( this.connection, ['query', args] );
	}
